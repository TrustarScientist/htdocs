setTimeout(() => {
    ...
    $(".profile-path-btns .communities").click(function() {
        $(".paths-con .communities-path").css("display", "block");
        $(".paths-con .about-path").fadeOut("fast");
        $(".paths-con .network-path").fadeOut("fast");
        $(".paths-con .posts-path").fadeOut("fast");
    ...
    $.post("/user/profile", {}, function(data, status) {
        ...
        //lets store profile id in post modal...for later processing
        document.querySelector(".edit-if-its-yours").dataset.cuser = profileDetails.cuser;
            ...
            if (pmfBody.value != "") {
                $.post("/message/send", msgData, function(data, status) {
                    ...
                    if (data == 1) {
                        ...
                        pmfBtn.textContent = "Message not sent...";
                    }
                });
            } else {
                pmfBtn.textContent = "...Can't Send an Empty Message";
            }
        });

        // code to allow profile messaging form display or not
        if ((profileDetails.can_message == true)) {
