$(document).ready(function() {
    /** niche details info fetching  page */
    $.post("/niche/@currentniche", {}, function(data, status) {
        //alert(data);
        let nicheObject = JSON.parse(data);
        document.querySelector(".niche-name").textContent = nicheObject.name;
        document.querySelector("#niche-banner").src = nicheObject.banner;
        document.querySelector("#niche-banner").style.display = "block";
        document.querySelector(".about-details-con .desc").textContent = nicheObject.description;
        document.querySelector(".about-details-con .members").textContent = nicheObject.members;
        document.querySelector(".about-details-con .posts").textContent = nicheObject.posts;
        document.querySelector(".about-details-con .parent-niche").textContent = nicheObject.parent_alias;
        document.querySelector(".about-details-con .parent-niche").href = "/niche/" + nicheObject.parent_alias;
        let mnPostsBtn = document.querySelector(".more-niche-posts-btn");
        mnPostsBtn.dataset.nid = nicheObject.id;
        // get post id and request for profile posts
        profilePosts("/niche/posts", {
            "niche_id": nicheObject.id,
            "starting": 0,
            "amount": 12,
        }, mnPostsBtn);
    ...
    $(".more-niche-posts-btn").click(function(e) {
        profilePosts("/niche/posts", {
            "niche_id": e.target.dataset.nid,
    ...
    //  helper function
    function profilePosts(url, dataset = "", ctrlBtn = {}) {
        $.post(
            url,
            dataset,
            function(data, status) {
                ...
                if (data != "[]") {
                    ...
                    responseData.forEach(postObject => {
                        ...
                        fNicheCon.setAttribute("class", "user-info");
                        ...
                        fAnchor.href = `/user/${postObject.user.username}`;
                        fAnchor.textContent = postObject.user.username;
                        ...
                        ctrlBtn.dataset.starting = (Number(ctrlBtn.dataset.starting) + 1);
